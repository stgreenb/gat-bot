...
function stopPoll(){
  ...
  */
  ...
  var userData = userInputSheet.getSheetValues(1, 2, 3, 1);

  // keep track of used lines in the logs spreadsheet
  var ssLines = 1;

  // log the bearer token and URL to the logs sheet
  logsSheet.getRange(ssLines, 1).setValue('Bearer token');
  logsSheet.getRange(ssLines, 2).setValue(userData[0][0]);
  ssLines++;
  logsSheet.getRange(ssLines, 1).setValue('Web App URL');
  logsSheet.getRange(ssLines, 2).setValue(userData[1][0]);
  ssLines++;
  ...
  if (notBot) {
    ...
    var postMessageOptions = {
      ...
      "payload": {
        ...
        "text" : "I got your answer: " + jsonGMD.text,
      }
    };
